...
namespace BetterJoyForCemu {
    public partial class MainForm : Form {
        ...
        private void foldLbl_Click(object sender, EventArgs e) {
            ...
            foldLbl.Text = rightPanel.Visible ? "<隐藏面板" : "配置面板>";
        }
        ...
        private void StartCalibrate(object sender, EventArgs e) {
            if (Program.mgr.j.Count == 0) {
                this.console.Text = "未发现设备，请连接一个需要校准Swwitch Pro手柄。";
                return;
            }
            if (Program.mgr.j.Count > 1) {
                this.console.Text = "一次只能校准一个控制器，请断开其它控制器连接。";
                return;
            }
        ...
        private void conCntrls_Enter(object sender, EventArgs e) {

        }

        private void CountDown(object sender, EventArgs e) {
            if (this.count == 0) {
                this.console.Text = "校准中，请稍等......";
                countDown.Stop();
                this.StartGetData();
            } else {
                this.console.Text = "请将控制器平放，并在校准结束前保持其水平。" + "\r\n";
                this.console.Text += "控制器校准将在" + this.count + "秒后自动开始。";
                this.count--;
            }
        }
