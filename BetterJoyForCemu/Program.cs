        ...
        void CleanUp() { // removes dropped controllers from list
            ...
            for (int i = 0; i < j.Count; i++) {
                ...
                if (v.state == Joycon.state_.DROPPED) {
                    ...
                    form.AppendTextBox("已移除离线的控制器。已便重新连接。\r\n");
                }
            }
            ...
            while (ptr != IntPtr.Zero) {
                ...
                if (validController && !ControllerAlreadyAdded(enumerate.path)) {
                    switch (prod_id) {
                        case product_l:
                            isLeft = true;
                            form.AppendTextBox("左Joy-Con手柄已连接。\r\n"); break;
                        case product_r:
                            isLeft = false;
                            form.AppendTextBox("右Joy-Con手柄已连接。\r\n"); break;
                        case product_pro:
                            isLeft = true;
                        ...
                        default:
                            form.AppendTextBox("非任天堂Joy-Con输入设备，已跳过。\r\n"); break;
                    }
                    ...
                    if (Program.useHIDG) {
                        ...
                        try {
                            ...
                            form.AppendTextBox("无法此将控制器添加到阻止列表。\r\n");
                        }
                    }
                    // -------------------- //
                    ...
                    try {
                        ...
                        form.AppendTextBox("无法打开设备路径！请确认操作系统的版本（64位与32位），并检查是否打开了对应的版本。\r\n");
                        break;
                    }
            ...
            if (useHIDG) {
                ...
                if (Boolean.Parse(ConfigurationManager.AppSettings["PurgeWhitelist"])) {
                    try {
                        ...
                        form.console.AppendText("无法移除白名单。\r\n");
                        useHIDG = false;
                    }
                }
        ...
        public static void Stop() {
            if (Program.useHIDG) {
                try {
                    ...
                    form.console.AppendText("无法将程序从白名单中移除。\r\n");
                }
            }
        ...
        static void Main(string[] args) {
            using (Mutex mutex = new Mutex(false, "Global\\" + appGuid)) {
                if (!mutex.WaitOne(0, false)) {
                    MessageBox.Show("已有一个BetterJoy程序正在运行。", "BetterJoy正在运行中");
                    return;
                }
